...
class Channel extends EventEmitter {
  ...
  _taskWorker(task, done) {
    ...
    if (this.state === 'closed' && this.connection.state === 'open') {
      ...
      if (type === 'method') {
        ...
        this.connection._sendBody(this.channel, data, options, (err, res) => { /* ignore err */ })
        if (okMethod == null) doneFn()
  ...
  _callOutstandingCallbacks(message = new Error("Channel Unavaliable")) {
    ...
    for (const [key, cbs] of outStandingCallbacks.entries()) {
      for (const cb of cbs) {
        cb(message)
      }
    }
  ...
  _onChannelMethod(channel, method, args) {
    ...
    if (channel !== this.channel) {
      return debug(1, () => ["channel was sent to the wrong channel object", channel, this.channel])
    }
