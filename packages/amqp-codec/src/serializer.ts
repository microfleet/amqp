...
const debug = require('debug')('amqp:serializer')
...
function serializeShortString(serializer: Serializer, input: string, field: Field) {
  if (typeof input !== 'string') {
    throw new TypeError(`"${field.name}" param must be a string`)
  }
...
function encodeHeader(serializer: Serializer, channel: number, { size, properties }: Omit<ContentHeader, 'classInfo' | 'weight'>): Buffer {
  debug('serialize %j', properties)
  ...
  serializeInt8(serializer, size) // byte size of body
  ...
  for (const [i, field] of classInfo.fields.entries()) {
    ...
    if (hasOwnProperty.call(properties, field.name)) {
  ...
  serializeFields(serializer, classInfo.fields, properties, false)
